...
public class SingleTagVisionPoseEstimator extends Command {
  ...
  public void execute() {
    ...
    if (RobotContainer.vision.getLatestLeftResult().getMultiTagResult().estimatedPose.isPresent) {
      ...
      try {
        if (RobotContainer.vision.getLatestLeftResult().getBestTarget().getPoseAmbiguity() <= 0.05) {
      ...
      try {
        if (RobotContainer.vision.getLatestRightResult().getBestTarget().getPoseAmbiguity() <= 0.05) {
